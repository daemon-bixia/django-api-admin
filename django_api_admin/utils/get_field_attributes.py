...
def get_field_attributes(name, field, serializer, model_admin, change):
    ...
    for attr_name in model_admin.serializer_field_attributes[form_field["type"]]:
        ...
        if attr_name == "default" and getattr(attr, "__name__", None) == "empty":
            value = False if type(field).__name__ == "BooleanField" else None
        ...
        # default to iso-8601
            ...
            if type(field).__name__ == "DateField":
                value = (
                    humanize_datetime.date_formats(attr).split(", ")
                    if attr
                    else humanize_datetime.date_formats(["iso-8601"]).split(", ")
                )
            elif type(field).__name__ == "DateTimeField":
                value = (
                    humanize_datetime.datetime_formats(attr).split(", ")
                    if attr
                    else humanize_datetime.datetime_formats(["iso-8601"]).split(", ")
                )
            elif type(field).__name__ == "TimeField":
                value = (
                    humanize_datetime.time_formats(attr).split(", ")
                    if attr
