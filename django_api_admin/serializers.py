...
class PaginationSerializer(serializers.Serializer):
    num_pages = serializers.IntegerField(required=True, help_text=_("The total number of pages."))
    count = serializers.IntegerField(required=True, help_text=_("The total number of items."))
    has_next = serializers.BooleanField(required=True, help_text=_("Whether there is a next page."))
...
class ModelSerializer(serializers.Serializer):
    ...
    perms = PermissionsSerializer(help_text=_("The permissions for the model."))
    view_only = serializers.BooleanField(help_text=_("Whether the model is view-only."))


class AppSerializer(serializers.Serializer):
    name = serializers.CharField(help_text=_("The name of the application."))
    app_label = serializers.CharField(help_text=_("The label of the application."))
    app_url = serializers.CharField(help_text=_("The URL of the application."))
    has_module_perms = serializers.BooleanField(help_text=_("Whether the user has permissions for this application."))
...
class AutoCompleteSerializer(serializers.Serializer):
    app_label = serializers.CharField(required=True, help_text=_("The app label of the model to search."))
    model_name = serializers.CharField(required=True, help_text=_("The name of the model to search."))
    field_name = serializers.CharField(
        required=True,
        help_text=_("The name of the source field to use for the search."),
    )
    term = serializers.CharField(required=False, default="", help_text=_("The search term to filter results."))


class AutocompleteResultSerializer(serializers.Serializer):
    id = serializers.CharField(help_text=_("The unique identifier of the object."))
    text = serializers.CharField(help_text=_("The display text of the object."))
...
class FormatsSerializer(serializers.Serializer):
    ...
    DATE_INPUT_FORMATS = serializers.ListField(child=serializers.CharField(allow_blank=False))
    TIME_INPUT_FORMATS = serializers.ListField(child=serializers.CharField(allow_blank=False))
    DATETIME_INPUT_FORMATS = serializers.ListField(child=serializers.CharField(allow_blank=False))
...
class FieldAttributesSerializer(serializers.Serializer):
    ...
    html_cutoff = serializers.IntegerField(
        help_text=(
            "The maximum number of choices that will be rendered in HTML select dropdowns to prevent performance degradation."
        )
    )
    html_cutoff_text = serializers.CharField(
        help_text=(
            "The string message displayed in HTML forms when the total available choices exceed the html_cutoff threshold."
        )
    )
    ...
    allow_empty = serializers.BooleanField(
        help_text=(
            "If True, allows collection-based fields"
            " (like ListField or ManyRelatedField) to pass validation with zero elements."
        )
    )
    child = serializers.JSONField(
        help_text=(
            "The configuration metadata representing"
...
class PrepopulatedFieldsSerializer(serializers.Serializer):
    mapping = serializers.DictField(
        child=serializers.ListField(
            ...
            help_text=_("List of source field names whose values are concatenated to populate the target field."),
        ),
        help_text=_(
...
class FormSerializer(serializers.Serializer):
    ...
    fieldsets = serializers.ListField(
        ...
        help_text=_("Admin fieldsets configuration matching Django admin's fieldsets structure."),
        required=True,
    )
    ...
    save_as = serializers.BooleanField(required=True, help_text="enable a “save as new” feature on admin change forms")
...
class InlineSerializer(serializers.Serializer):
    ...
    formset = serializers.ListField(
        ...
        required=True,
        help_text=_("The formset configuration for the model a list of lists of field definitions."),
    )
...
class SiteContextSerializer(serializers.Serializer):
    site_title = serializers.CharField(help_text=_("The title of the admin site."))
    site_header = serializers.CharField(help_text=_("The header of the admin site."))
...
class ActionChoiceSerializer(serializers.Serializer):
    action = serializers.CharField(help_text=_("The internal name of the action."))
    description = serializers.CharField(help_text=_("The human-readable description of the action."))


class FilterChoiceSerializer(serializers.Serializer):
    selected = serializers.BooleanField(help_text=_("Whether this filter choice is currently selected."))
    query_string = serializers.CharField(help_text=_("The query string to apply this filter choice."))
    display = serializers.CharField(help_text=_("The human-readable label for this filter choice."))


class FilterSerializer(serializers.Serializer):
    title = serializers.CharField(help_text=_("The title of the filter."))
    choices = FilterChoiceSerializer(many=True, help_text=_("The list of available choices for this filter."))
...
class EditingFieldSerializer(serializers.Serializer):
    ...
    attrs = serializers.DictField(help_text=_("The attributes and configuration for the editing field."))


class ConfigSerializer(serializers.Serializer):
    actions_on_top = serializers.BooleanField(help_text=_("Whether to display actions at the top of the list."))
    actions_on_bottom = serializers.BooleanField(help_text=_("Whether to display actions at the bottom of the list."))
    actions_selection_counter = serializers.BooleanField(help_text=_("Whether to show a counter for selected items."))
    empty_value_display = serializers.CharField(help_text=_("The string to display for empty values."))
    list_display = serializers.ListField(child=serializers.CharField(), help_text=_("The fields to display in the list."))
    ...
    exclude = serializers.ListField(child=serializers.CharField(), help_text=_("The fields to exclude from the list."))
    show_full_result_count = serializers.BooleanField(help_text=_("Whether to show the total count of results."))
    list_per_page = serializers.IntegerField(help_text=_("The number of items to show per page."))
    list_max_show_all = serializers.IntegerField(
        help_text=_("The maximum number of items to show when 'Show all' is clicked.")
    )
    date_hierarchy = serializers.CharField(help_text=_("The field to use for date-based navigation."))
    search_help_text = serializers.CharField(allow_null=True, help_text=_("The help text to display for the search box."))
    ...
    search_fields = serializers.ListField(child=serializers.CharField(), help_text=_("The fields to include in the search."))
    ...
    editing_fields = serializers.ListField(child=EditingFieldSerializer(), help_text=_("Metadata for list-editable fields."))


class ColumnSerializer(serializers.Serializer):
    field = serializers.CharField(help_text=_("The field name associated with this column."))
    headerName = serializers.CharField(help_text=_("The human-readable header name for this column."))


class RowSerializer(serializers.Serializer):
    id = serializers.IntegerField(help_text=_("The primary key of the object."))
